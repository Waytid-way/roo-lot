...
ผลการทดลองพบว่าโมเดล **Random Forest** ให้ประสิทธิภาพดีที่สุด ด้วยค่าสัมประสิทธิ์การตัดสินใจ (**R²**) สูงถึง **0.9888** และมีความคลาดเคลื่อนสัมบูรณ์เฉลี่ย (**MAE**) เพียง **14.58 kWh** (ประมาณ 61 บาท) โดยปัจจัยที่มีอิทธิพลต่อค่าไฟมากที่สุดคือจำนวนสมาชิกในครัวเรือน (Household Size) คิดเป็น 68.6% ตามด้วยการมีเครื่องปรับอากาศ (Has AC) 30.5% และฤดูกาล/วันหยุดมีผลกระทบเล็กน้อย (<1%) อย่างไรก็ตาม พบข้อจำกัดเรื่องตัวแปรฤดูกาลที่ไม่ส่งผลต่อการทำนายเท่าที่ควร เนื่องจากการสร้างข้อมูลวันที่จำลองขึ้น (Synthetic Date Distribution)
...
> **ผลการดำเนินงาน**: ✅ **บรรลุเป้าหมาย** โดยได้โมเดลที่มี R² = 0.9888 (เกินเป้า 16%) และ MAE = 14.58 kWh (ดีกว่าเป้า 71%) ตามที่ระบุในบทที่ 4.2
...
**Metrics บน Test Set:**
- **R² Score**: 0.9888 (98.88% - การอธิบายความแปรปรวนของข้อมูล)
- **MAE (Mean Absolute Error)**: 14.58 kWh (~61 บาท/เดือน)
- **RMSE (Root Mean Squared Error)**: 18.56 kWh (~78 บาท/เดือน)

**การตีความผลลัพธ์:**

ค่า R² ที่ 0.9888 แสดงว่าโมเดลสามารถอธิบายความแปรปรวนของข้อมูลได้ถึง 98.88% ซึ่งถือว่าเป็นความแม่นยำในระดับสูงมาก

ค่า MAE ที่ 14.58 kWh (หรือประมาณ 61 บาท) หมายความว่า โดยเฉลี่ยแล้ว การทำนายของโมเดลจะคลาดเคลื่อนจากค่าจริงประมาณ 15 kWh ต่อเดือน ซึ่งอยู่ในระดับที่ยอมรับได้สำหรับการใช้งานจริง (คิดเป็นประมาณ 3-5% ของค่าไฟฟ้าเฉลี่ยต่อเดือน)
...
- **R² = 0.9888**: โมเดลอธิบายความแปรปรวนได้ 98.88% (ใกล้เคียง Perfect Model)
- **MAE = 14.58**: เฉลี่ยแล้วคลาดเคลื่อน 14.58 kWh หรือประมาณ 61 บาท/เดือน
...
3. **หน้าแสดงผลลัพธ์ (Result Card)**:
   ...
   - **ความแม่นยำของโมเดล**: แสดง R² Score = 98.88%
   - **ความคลาดเคลื่อนโดยเฉลี่ย**: ±61 THB ต่อเดือน
...
โปรเจค Roo-Lot ประสบความสำเร็จตามวัตถุประสงค์ที่ตั้งไว้ สามารถสร้างระบบทำนายค่าไฟด้วยโมเดล Random Forest ที่มีความแม่นยำสูง (**R² = 0.9888**) และความคลาดเคลื่อนอยู่ในระดับที่ยอมรับได้ (**MAE = 14.58 kWh หรือ ~61 บาท**) ซึ่งดีกว่าเป้าหมายที่ตั้งไว้มาก ระบบได้รับการพัฒนาพร้อม Web Application และพร้อมใช้งานจริง ช่วยให้ผู้ใช้สามารถประเมินค่าใช้จ่ายได้ง่ายและรวดเร็ว
...
ระบบปัจจุบันใช้ features เพียง 5 ตัว (`household_size`, `has_ac`, `season_hot`, `season_rainy`, `weekend_ratio`) ซึ่งถึงแม้จะให้ความแม่นยำสูง (R² = 98.88%) แต่ยังไม่ครอบคลุมปัจจัยอื่นๆ เช่น:
